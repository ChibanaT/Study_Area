...
//----------------------------
LinearResult gauss_seidel(const std::vector<std::vector<double>>& A_in, const std::vector<double>& b_in, int max_iter, double tol) {
    ...
    for (int i = 0; i < n; ++i) {
        if (std::fabs(A_in[i][i]) < 1e-12)
            throw std::runtime_error("Zero diagonal element encountered in Gauss-Seidel method");
    }
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        // Compute new approximation using updated values
        for (int i = 0; i < n; ++i) {
            double sum = 0.0;
            for (int j = 0; j < n; ++j) {
                if (i != j) {
            ...
            result.solution[i] = (b_in[i] - sum) / A_in[i][i];
